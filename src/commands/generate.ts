...
import { SHELL_SHEBANG, SHELL_SCRIPT_EXT } from '../core/prompt';
...
import { StepExecutor, getPowerShellPrelude, getPowerShellEpilogue } from '../core/executor';
...
/** Render steps as a complete saveable script file: shebang, error pragma
 *  (set -euo pipefail / $ErrorActionPreference='Stop'), provenance comment,
 *  then per-step comments + commands. fish has no portable strict-mode flag
 *  so the err pragma is omitted there (the AI is instructed to add per-step
 *  guards).
 *
 *  PowerShell additionally:
 *    1. prepends a UTF-8 BOM so PS 5.1 reads here-strings as UTF-8 instead
 *       of the system ANSI codepage (GBK on Chinese Windows would corrupt
 *       Chinese / emoji characters in heredocs at parse time)
 *    2. injects the wts UTF-8 wrapper (Set-Content / Out-File / Add-Content
 *       overrides) so target files written by the script are BOM-less UTF-8
 *       regardless of the AI's cmdlet choice. Without this, the original
 *       cmdlets fall back to system ANSI on PS 5.1 and break browsers /
 *       Node tooling that expect UTF-8.
 *  Both fixes mirror what the in-session executor already does for ad-hoc
 *  step execution. */
function formatScriptForSave(steps: Step[], shell: ShellType): string {
  const shebang = SHELL_SHEBANG[shell];
  const provenance = `# Generated by wts on ${new Date().toISOString().slice(0, 10)}`;
  const body = formatScriptForCopy(steps);

  if (shell === 'powershell') {
    // U+FEFF prepended to file = UTF-8 BOM bytes (EF BB BF) — required so PS
    // 5.1 reads the .ps1 as UTF-8 instead of the system ANSI codepage.
    const BOM = '﻿';
    return [
      BOM + shebang,
      provenance,
      '',
      '# wts UTF-8 wrapper: ensures Set-Content / Out-File / Add-Content emit',
      '# BOM-less UTF-8 (PS 5.1 otherwise writes system ANSI / GBK on Chinese',
      '# Windows, which breaks browsers, Node JSON.parse, etc.).',
  ...
  const errPragma = (shell === 'bash' || shell === 'zsh')
    ? 'set -euo pipefail'
    : '';
  const header = [shebang, errPragma].filter(Boolean).join('\n');
  return `${header}\n${provenance}\n\n${body}\n`;
}

/** Save the generated script to a file in cwd. Default filename is
 *  wts-script-<YYYY-MM-DD-HHMM>.<ext>; user can overwrite the prompt to
 *  pick their own name. Confirms before overwriting an existing file.
 *  On non-Windows + non-PowerShell, chmods the file to 0o755 so it's
 *  executable; chmod failure is non-fatal (some filesystems don't honor it). */
async function saveScript(steps: Step[], shell: ShellType): Promise<void> {
  const prompts = await import('@inquirer/prompts');
  const ext = SHELL_SCRIPT_EXT[shell];
  // 2026-05-02T14:30:45.000Z -> 2026-05-02-14-30
  const ts = new Date().toISOString().replace(/[:T]/g, '-').slice(0, 16);
  const defaultName = `wts-script-${ts}.${ext}`;

  let filename: string;
  try {
    filename = await prompts.input({ message: 'Save as:', default: defaultName });
  } catch (err: any) {
    if (err?.name === 'ExitPromptError' || err?.message?.includes('User force closed')) {
  ...
  if (!filename) {
    console.log(`  ${chalk.gray('Save cancelled (empty filename)')}`);
    return;
  }
  ...
  if (fs.existsSync(targetPath)) {
    let confirm = false;
    try {
      const display = path.relative(process.cwd(), targetPath) || filename;
      confirm = await prompts.confirm({
        message: `${display} already exists. Overwrite?`,
        default: false,
      });
    } catch {
      confirm = false;
  ...
  const content = formatScriptForSave(steps, shell);
  try {
    fs.writeFileSync(targetPath, content, { encoding: 'utf8' });
    if (shell !== 'powershell' && process.platform !== 'win32') {
      try { fs.chmodSync(targetPath, 0o755); } catch { /* non-fatal */ }
    }
    const display = path.relative(process.cwd(), targetPath) || filename;
    await displaySuccess(`Saved to ${display}`);
  } catch (err: any) {
    await displayError(`Failed to save: ${err.message}`);
  }
}

/** Copy the full script to the clipboard in paste-ready form. */
async function copyScriptToClipboard(steps: Step[]): Promise<void> {
  const text = formatScriptForCopy(steps);
  const ok = await copyToClipboard(text);
  if (ok) {
    await displaySuccess(`Copied ${steps.length} steps to clipboard`);
  } else {
    await displayError('Failed to copy to clipboard');
  }
}
...
async function interactiveScriptMenu(
  ...
  const choices = [
    ...
    { name: `${chalk.cyan('Save')} script as a file`, value: 'save' },
    { name: `${chalk.cyan('Copy')} script to clipboard`, value: 'copy' },
    { name: `${chalk.gray('Cancel')}`, value: 'cancel' },
  ];
  ...
  switch (action) {
    ...
    case 'improve':
      ...
      break;
    case 'save':
      await saveScript(steps, shell);
      // Save is a side action — return to the menu so the user can still Run / Copy
      await interactiveScriptMenu(client, originalDescription, shell, config, steps, risk);
      break;
    case 'copy':
      await copyScriptToClipboard(steps);
      await interactiveScriptMenu(client, originalDescription, shell, config, steps, risk);
      break;
    case 'cancel':
    default:
